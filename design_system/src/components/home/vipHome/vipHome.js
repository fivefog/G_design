export default {
    ...
    methods: {
        ...
        async fetchMenuList() {
            try {
                let { data } = await this.$http.get('/menu/tree/vip')
        ...
        handleCommand(command) {
            ...
            //退出
                ...
                this.$confirm('您确定要退出系统吗?', '温馨提示', {
                    ...
                    //清除token
                    localStorage.removeItem('vipAuthorization')
    ...
    watch: {
        $route(to) {
            ...
            tag.forEach(item => {
                //不是路径首页才能放入tags中
                if (item.path !== `/viphome/${this.$route.params.id}/welcome`) {
                    this.tags.push(item)
                }
            })
            ...
            this.tags.forEach((item, index, arr) => {
                ...
                if (this.nowId === index && to.path !== `/viphome/${this.$route.params.id}/welcome`) {
