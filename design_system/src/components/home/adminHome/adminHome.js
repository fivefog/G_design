export default {
    ...
    methods: {
        ...
        async fetchMenuList() {
            try {
                let { data } = await this.$http.get('/menu/tree/admin')
    ...
    watch: {
        $route(to) {
            ...
            tag.forEach(item => {
                //不是路径首页才能放入tags中
                if (item.path !== `/adminhome/${this.$route.params.id}/welcome`) {
                    this.tags.push(item)
                }
            })
            ...
            this.tags.forEach((item, index, arr) => {
                ...
                if (this.nowId === index && to.path !== `/adminhome/${this.$route.params.id}/welcome`) {
