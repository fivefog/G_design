    ...
    methods: {
            ...
            if (this.multSelectedId.length === 0) {
                ...
                this.$confirm('您确定批量删除管理员信息吗?', '温馨提示', {
                    ...
                    try {
                        let {
                            data
                        } = await this.$http.delete(`/adminlist/multId/${this.ids}`)
                        if (data.status === 1) {
                            //当已全选时
        ...
        async search() { //获取根据条件搜索管理员的请求
            ...
            try {
                let {
                    data
                } = await this.$http.get(`/adminlist/${pagenum}/${pagesize}`, {
                        params: {
                            s_id,
                            username,
                            name,
        ...
        async showEdit(id) { //根据id展示需要修改的管理员信息
            try {
                let {
                    data
        ...
        editAdmin() { //修改管理员信息
            let { username, name, password, sex, age, xueli, email, phone } = this.editForm //解构修改管理员信息表单数据
        ...
        handleDelete(id) { //根据id删除管理员
            this.calculate //计算除数与取余数的属性
            let { pagesize, pagenum, remainNum, divisor } = this.query
            this.$confirm('您确定删除该管理员信息吗?', '温馨提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
                ...
                try {
                    let {
                        data
                    } = await this.$http.delete(`/adminlist/${id}`)
                    if (data.status === 1) {
