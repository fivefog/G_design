export default {
    data() {
        ...
        return {
            ...
            dialogFormVisibleEdit: false, //控制修改售货员信息对话框的显示与隐藏
        }
    },
    methods: {
        ...
        multDelete() { //批量删除按钮
            if (this.multSelectedId.length === 0) {
                ...
                this.$confirm('您确定批量删除售货员信息吗?', '温馨提示', {
                    ...
                    try {
                        let {
                            data
                        } = await this.$http.delete(`/salesmanlist/multId/${this.ids}`)
                        if (data.status === 1) {
                            //当已全选时
        ...
        async search() { //获取根据条件搜索售货员的请求
            ...
            let s_id, username, name, password, sex, age, xueli, email, phone, createtime
            if (filters === 's_id') {
                s_id = search
            ...
            try {
                let {
                    data
                } = await this.$http.get(`/salesmanlist/${pagenum}/${pagesize}`, {
                        params: {
                            s_id,
                            username,
                            name,
                ...
                if (data.status === 1 && (s_id == '' || username == '' || name == '' || password == '' || sex == '' || age == '' || xueli == '' || email == '' || phone == '' || createtime == '' || filters == '')) {
                    this.fetchAllPage() //获取分页和总条数的请求  
                    this.$message.warning('查询条件或查询内容不能为空')
                } else {
                    this.searchPage = true
                    if (data.status === 1) {
                        this.salesmanList = data.data[0] //分页
                        ...
                        this.$message.error('抱歉！无符合此条件的售货员信息！请重新输入')
                        this.fetchAllPage() //获取分页和总条数的请求  
                    }
                }
            } catch (error) {
        ...
        getSalesmanList() { //搜索框清空后获取售货员列表数据
            this.fetchToolPage() //获取分页和总条数的请求
        },
        ...
        async showEdit(id) { //根据id展示需要修改的售货员信息
            try {
                let {
                    data
                } = await this.$http.get(`/myselfquery/${id}`)
        ...
        editSalesman() { //修改售货员信息
            let { username, name, password, sex, age, xueli, email, phone } = this.editForm //解构修改售货员信息表单数据
            this.$refs.editFormRef.validate(async(valid) => { //校验
                if (valid) { //校验成功获取修改售货员信息的请求
                    try { //调用修改售货员信息请求
                        let { data } = await this.$http.patch(`/myselfupdate/${this.editForm.id}`, {
                                username,
                                name,
                                password,
                                sex,
        ...
        editDialogClosed() { //监听修改售货员信息对话框的关闭事件
            this.$refs.editFormRef.clearValidate() //移除校验结果
        },

        handleDelete(id) { //根据id删除售货员
            this.calculate //计算除数与取余数的属性
            let { pagesize, pagenum, remainNum, divisor } = this.query
                // console.log(pagenum)
                // console.log(divisor)
                // console.log(remainNum)
            this.$confirm('您确定删除该售货员信息吗?', '温馨提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
                ...
                try {
                    let {
                        data
                    } = await this.$http.delete(`/salesmanlist/${id}`)
                    if (data.status === 1) {
