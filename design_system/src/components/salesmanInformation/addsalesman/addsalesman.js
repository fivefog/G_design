export default {
    ...
    methods: {
        goBack() { //返回售货员列表
            this.$router.push('/superadminhome/salesmanlist')
        },
        //获取随机员工号
        fetchS_id() {
            ...
            let s_id = id1 + id2
            this.addSalesmanForm.s_id = parseInt(s_id)
                // console.log(this.addSalesmanForm.s_id)
        },
        //获取当前日期
        fetchDate() {
            this.addSalesmanForm.createtime = this.$moment().format('YYYY-MM-DD')
                // console.log(this.addSalesmanForm.createtime)
        },
        ...
        async addSalesman() { //添加售货员信息
            let {
                ...
                phone,
                createtime
            } = this.addSalesmanForm

            this.$refs.addSalesmanRef.validate(async(valid) => { //校验
                if (valid) { //校验成功获取添加售货员信息信息的请求
                    try {
                        let {
                            data
                        } = await this.$http.post('/addsalesman', {
                            s_id,
                            username,
                            name,
                            password,
                        ...
                        if (data.status === 1) { //状态码为1时添加成功
                            ...
                            this.$refs.addSalesmanRef.resetFields() //对添加售货员信息表单进行重置，将其值重置为初始值并移除校验结果
