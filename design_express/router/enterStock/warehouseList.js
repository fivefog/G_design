...
router.get('/total', async(req, res) => {
    let sql = `select count(*) as total from warehouse where `
...
router.get('/', async(req, res) => {
    let sql = `select w_name from warehouse`
    try {
        let result = await query(sql)
...
router.get('/:pagenum/:pagesize', async(req, res) => {
    ...
    let sql1 = `select * from warehouse where `
    for (let key in req.query) {
    ...
    let sql2 = `select count(*) as total  from warehouse where ` //计算总条数
    for (let key in req.query) {
...
router.get('/:id', async(req, res) => {
    let { id } = req.params
    let sql = `select * from warehouse where id=${id}`
...
router.delete('/:id', async(req, res) => {
    let { id } = req.params
    let sql = `delete from warehouse where id=${id}`
    try {
        await query(sql)
...
router.delete('/multId/:ids', async(req, res) => {
    let { ids } = req.params
    let sql = `delete from warehouse where id in ${ids}`
