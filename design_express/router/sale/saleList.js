...
router.get('/total', async(req, res) => {
    let sql = `select count(*) as total from sale where `
...
router.get('/:pagenum/:pagesize', async(req, res) => {
    ...
    let sql1 = `select * from sale where `
    for (let key in req.query) {
    ...
    let sql2 = `select count(*) as total  from sale where ` //计算总条数
    for (let key in req.query) {
...
router.get('/:id', async(req, res) => {
    let { id } = req.params
    let sql = `select * from sale where id=${id}`
...
//通过id删除销售信息和服务器已上传的图片
router.delete('/:id', async(req, res) => {
    let { id } = req.params
    let sql = `select g_imgurl from sale where id=${id}`
    let sql1 = `delete from sale where id=${id}`
...
//批量删除销售信息和服务器已上传的图片(多选)
router.delete('/multId/:ids', async(req, res) => {

    let { ids } = req.params
    let sql = `select g_imgurl from sale where id in ${ids}`
    let sql1 = `delete from sale where id in ${ids}`
