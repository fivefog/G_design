...
router.get('/total', async(req, res) => {
    let sql = `select count(*) as total from downshelves where `
...
router.get('/', async(req, res) => {
    let sql = `select g_id from downshelves group by g_id`
    try {
        let result = await query(sql)
...
router.get('/query', async(req, res) => {
    let sql = `select * from downshelves where `
    for (let key in req.query) {
        sql += `${key}='${req.query[key]}'`
    }
...
router.get('/:pagenum/:pagesize', async(req, res) => {
    ...
    let sql1 = `select * from downshelves where `
    for (let key in req.query) {
    ...
    let sql2 = `select count(*) as total  from downshelves where ` //计算总条数
    for (let key in req.query) {
...
router.get('/:id', async(req, res) => {
    let { id } = req.params
    let sql = `select * from downshelves where id=${id}`
...
router.patch('/:id', async(req, res) => {
    let { id } = req.params
    let sql = `update downshelves set `
...
//通过id删除下架商品和服务器已上传的图片
router.delete('/:id', async(req, res) => {
    let { id } = req.params
    let sql = `select g_imgurl from downshelves where id=${id}`
    let sql1 = `delete from downshelves where id=${id}`
...
//批量删除下架商品和服务器已上传的图片(多选)
router.delete('/multId/:ids', async(req, res) => {

    let { ids } = req.params
    let sql = `select g_imgurl from downshelves where id in ${ids}`
    let sql1 = `delete from downshelves where id in ${ids}`
